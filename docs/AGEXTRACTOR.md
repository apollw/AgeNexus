...
Ao iniciar a extração, o navegador envia as coordenadas da imagem original junto dos bytes mantidos em memória. Cada categoria é executada isoladamente com `--categoria`; seu fragmento JSON bem-sucedido permanece na memória da página enquanto as demais são processadas. Uma falha não elimina categorias concluídas, e uma nova tentativa executa somente as pendentes. O administrador também pode descartar todos os fragmentos e recomeçar mantendo as imagens e delimitações.

O servidor valida a assinatura do arquivo e os pontos, cria uma pasta temporária com nome interno e um `regioes.json`, executa `python -m agextractor.interfaces.server` e recebe o fragmento por `stdout`. A pasta é removida depois de cada categoria. Apenas uma categoria é processada por vez em cada instância do AgeNexus e cada tentativa tem limite de cinco minutos. Quando as cinco terminam, `AgeExtractorJsonComposer` consolida os fragmentos no contrato original antes da revisão.
